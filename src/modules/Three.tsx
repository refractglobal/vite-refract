...
export function Instances({ children, ...props }) {
    const { nodes } = useGLTF(
        "https://rawcdn.githack.com/refractglobal/refract-asset/ac5c6c3ff9bf59b4d04a0d4719e6fdf809dd33f1/computers_1-transformed.glb"
    )
    ...
    return (
        <Screen {...props}>
            <PerspectiveCamera
                ...
                position={[0, 0, 15]}
            />
            <color attach="background" args={[invert ? "black" : "#00E0FF"]} />
...
function Leds({ instances }) {
    ...
    useFrame((state) => {
        ref.current.children.forEach((instance) => {
            ...
            const t = Math.round(
                (1 + Math.sin(rand * 10000 + state.clock.elapsedTime * rand)) /
                    2
            )
        ...
        <>
            <div
                style={{
                    ...
                    pointerEvents: "none",
                    background:
                        "linear-gradient(180deg, rgba(2,0,36,0) 0%, rgba(9,9,121,0) 64%, rgba(0,0,0,1) 100%)",
                }}
            ></div>
            <Canvas
                shadows
                dpr={[1, 1]}
                camera={{ position: [-1.5, 5, 10], fov: 45, near: 1, far: 20 }}
